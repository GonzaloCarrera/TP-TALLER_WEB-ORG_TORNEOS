...
=======
	<div class="container">
		<ul class='nav navbar-nav navbar-right' id="loggeo">
			<c:set var="usuario" value="${usuario}" scope="session" />
			<c:if test="${empty usuario.username}">
				<li><a href='registrar'><span
		...
		<a href='mis-torneos?idUsuario=<c:out value="${usuario.id}" />'	class="btn btn-primary" role="button"> Mis torneos </a> 
		<a href='registrar-equipo' class="btn btn-primary" role="button"> 	Registrar Equipo </a> 
		<a href='listado-torneo-inscripcion-abierta' class="btn btn-primary" role="button"> Inscribirme a torneo </a>
		<a href='seleccionar-horario?idUsuario=<c:out value="${usuario.id}" />'	class="btn btn-primary" role="button"> Seleccionar Horario </a> 
		<a href='proximos-partidos?idUsuario=<c:out value="${usuario.id}" />' class="btn btn-primary" role="button"> Proximos partidos </a>
	</div>
	<!-- Placed at the end of the document so the pages load faster -->
>>>>>>> e3cfe9c0375b55ece20085b0eb13d3fdd5640542
